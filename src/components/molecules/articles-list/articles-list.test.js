...
test('renders articles list loading state', () => {
  render(<ArticlesList  loading={true} />);
  const articlesLoading = screen.getByTestId("loading-articles");
  expect(articlesLoading).toBeInTheDocument();
});


test('renders articles list error state', () => {
    render(<ArticlesList  error="Error fetching data..." />);
    const articlesLoadingError = screen.getByTestId("error-loading-data");
    expect(articlesLoadingError).toBeInTheDocument();
  });
  

test('renders articles list', () => {
    render(<ArticlesList  articles={articlesTestData} error=""/>);
    const article = screen.getByTestId(`article-${articlesTestData[0].id}`);
